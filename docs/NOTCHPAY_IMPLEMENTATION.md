...
- `frontend/` — App React (Vite), déployée sur **Vercel**. Root Directory = `frontend` dans les paramètres Vercel.
- `server/` — API Express + **notchpay-js** (Option B), déployée sur **Railway**.

👉 **Pour les étapes externes (compte NotchPay, Vercel, Railway, webhooks):** voir [GUIDE_PAS_A_PAS.md](./GUIDE_PAS_A_PAS.md).
...
**Backend (`server/.env`):**
- `NOTCHPAY_SECRET_KEY` — Clé secrète (sk_test_... ou sk_live_...). Utilisée par le SDK **notchpay-js** (NotchPayAPI).
- `APP_URL` — URL du frontend (ex. https://votre-app.vercel.app) pour callback et CORS.

**Frontend (`frontend/.env`):**
- `VITE_API_URL` — URL de l'API backend (ex. https://votre-api.railway.app). En dev avec proxy Vite, laisser vide pour utiliser `http://localhost:3000`.

Voir `server/.env.example` et `frontend/.env.example`. Pour le déploiement pas à pas: [GUIDE_PAS_A_PAS.md](./GUIDE_PAS_A_PAS.md).

> **Important:** Les appels à l'API NotchPay sont faits uniquement depuis le **backend** (Option B). Le frontend appelle seulement votre API (`/api/votes/pay`, `/api/payments/:ref/status`).
